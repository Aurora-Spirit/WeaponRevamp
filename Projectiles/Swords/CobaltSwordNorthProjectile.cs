...
{
    public class CobaltSwordNorthProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < Main.projectile.Length; i++)
            {
                if (Main.projectile[i].active && Main.projectile[i].type == Projectile.type && Main.projectile[i].position != Projectile.position)
                {
                    ...
                    {
                        Dust pushDust = Dust.NewDustDirect(Vector2.Lerp(Projectile.Center, Main.projectile[i].Center, Main.rand.NextFloat()), 0, 0, DustID.TintableDustLighted, 0, 0, 0, new Color(1f, 0.75f, 1f));
                        pushDust.velocity *= 0.1f;
                    }
                }
                else if (Main.projectile[i].active && Main.projectile[i].type == ModContent.ProjectileType<Projectiles.Swords.CobaltSwordSouthProjectile>() && Main.projectile[i].position != Projectile.position)
                {
                    Projectile.velocity += (Projectile.position - Main.projectile[i].position) / (float)Math.Pow(Vector2.Distance(Projectile.position, Main.projectile[i].position)+1, 1.5) * -5;
                    if (Main.rand.NextBool(50, (int)Vector2.Distance(Projectile.position, Main.projectile[i].position) + 100))
                    {
            ...
            Dust passiveDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.TintableDustLighted, 0, 0, 0, new Color(1f, 0.75f, 1f));
            passiveDust.velocity *= 0.1f;
            passiveDust.velocity -= (passiveDust.position - Projectile.Center).RotatedBy(Math.PI / 2) * 0.05f;

            Lighting.AddLight(Projectile.position, new Vector3(0.5f, 0.375f, 0.5f));
