...
namespace WeaponRevamp.Projectiles.Spears
{
    public class MythrilHalberdProjectile : GlobalProjectile
    {
        private bool magicSwing = false;
        public override bool AppliesToEntity(Projectile projectile, bool lateInstatiation)
        {
            return projectile.type == ProjectileID.MythrilHalberd;
        }

        public override void OnSpawn(Projectile projectile, IEntitySource source)
        {
            Player player = Main.player[projectile.owner];
            magicSwing = player.CheckMana((int)(18f * player.manaCost), true, false);
            if (magicSwing) {
                player.manaRegenDelay = (int)player.maxRegenDelay;
                projectile.damage = (int)(projectile.damage * (1f - player.manaSickReduction));
            }
        ...
        public override void PostAI(Projectile projectile)
        {
            if(magicSwing)
            {
                Lighting.AddLight(projectile.Center, 0.6f, 0.9f, 1.1f);
                if (Main.rand.NextBool(2))
                {
                    int num14 = Dust.NewDust(projectile.position, projectile.width, projectile.height, DustID.BlueFlare, projectile.velocity.X * 0.2f, projectile.velocity.Y * 0.2f, 200, default(Color), 1.5f);
        ...
        public override void PostDraw(Projectile proj, Color lightColor)
        {
            if (magicSwing)
            {
                ...
                Color color1 = new Color(255, 255, 255, 0) * 0.5f;

                Color color = new Color(40, 100, 255, 0);
        ...
        public override bool? Colliding(Projectile projectile, Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            if (extensionBox.Intersects(targetHitbox) && magicSwing)
            {
        ...
        public bool AI_019_Spears_GetExtensionHitbox(out Rectangle extensionBox, Projectile proj)
        {
            ...
            float num3 = 10f;
            float num4 = 60f; //length?
            float num5 = 10f;
            float num6 = 15f; //width?
