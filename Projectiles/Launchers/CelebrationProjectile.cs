...
{
    public class CelebrationProjectile : ModProjectile
    {
        ...
        public override void SendExtraAI(BinaryWriter writer)
        {
            ...
            writer.Write(ammoSprite);
            writer.WriteFlags(destroyTiles, cluster, exploded);
        }
        public override void ReceiveExtraAI(BinaryReader reader)
        {
            ...
            ammoSprite = reader.ReadInt32();
            reader.ReadFlags(out destroyTiles, out cluster, out exploded);
        }
        ...
        public override void AI()
        {
            ...
            if(Main.myPlayer == Projectile.owner && liquidType != 0 && (Projectile.wet || Projectile.lavaWet || Projectile.honeyWet || Projectile.shimmerWet))
            {
                Projectile.Kill();
            }
            ...
            num240 = Dust.NewDust(Projectile.position - Projectile.velocity*0.5f, 0, 0, DustID.Torch, 0f, 0f, 100, default(Color), 2f);
            Main.dust[num240].noGravity = true;
            Main.dust[num240].velocity += Projectile.velocity * -0.2f;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (cluster)
            {
                ...
                {
                    Projectile proj = (Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.position, new Vector2(radius*3,0).RotatedByRandom(Math.PI*2), ModContent.ProjectileType<CelebrationClusterProjectile>(), Projectile.damage / 2, Projectile.knockBack, Projectile.owner));
                    if (proj.ModProjectile is CelebrationClusterProjectile rocket)
                    {
                        rocket.radius = (int)((float)radius * 0.5);
                        rocket.fireType = fireType;
                        rocket.destroyTiles = destroyTiles;
                        
                    }
                    proj.timeLeft = Main.rand.Next(15, 30);
                }
            }
            if (Main.netMode != 1)
            {
                ...
                switch (liquidType)
                {
                    case 1:
                        ...
                        break;
                    case 2:
                        Projectile.Kill_DirtAndFluidProjectiles_RunDelegateMethodPushUpForHalfBricks(pt2, 3f, DelegateMethods.SpreadLava);
                        break;
                    case 3:
                        Projectile.Kill_DirtAndFluidProjectiles_RunDelegateMethodPushUpForHalfBricks(pt2, 3f, DelegateMethods.SpreadHoney);
                        break;
                    case 4:
                        Projectile.Kill_DirtAndFluidProjectiles_RunDelegateMethodPushUpForHalfBricks(pt2, 3.5f, DelegateMethods.SpreadDry);
                        break;
                }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Rectangle sourceRectangle = texture.Frame(4, 12);
            Vector2 origin = sourceRectangle.Size() / 2f;
            float scale = Projectile.scale;
            float lightingColor = Lighting.GetColor(Projectile.Center.ToTileCoordinates()).ToVector3().Length() / (float)Math.Sqrt(3.0);
            position = Projectile.Center - Main.screenPosition;
            Main.EntitySpriteDraw(texture, position, texture.Frame(4, 12, fireType, ammoSprite), new Color(255, 255, 255, 255) * lightingColor, Projectile.rotation, origin, scale, SpriteEffects.None, 0f);
