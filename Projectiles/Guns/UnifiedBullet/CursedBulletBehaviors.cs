...
namespace WeaponRevamp.Projectiles.Guns.UnifiedBullet
{
    ...
    public class CursedSmokeTrail : BulletBehavior
    {
        private Color dustColor = new Color(0.3f, 0.5f, 0.0f, 0.2f);
        ...
        public override void AI(ref Projectile projectile)
        {
            if (projectile.ai[1] > 5)
            {
                for (float i = 0; i < 2; i++)
                {
                    Dust piss = Dust.NewDustDirect(projectile.position + new Vector2(-2, -2), 0, 0, DustID.CursedTorch);
                    piss.velocity *= 0.8f;
                    piss.position += projectile.velocity * i / 2;
                    piss.velocity += projectile.velocity * 0.2f;
                    piss.scale = 0.9f;
    ...
    public class CursedFlameBlast : BulletBehavior
    {
        private int range;

        public CursedFlameBlast(int range)
        {
            this.range = range;
        }
        ...
        private void CursedExplosion(Projectile projectile)
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                Dust piss = Dust.NewDustDirect(projectile.position, 0, 0, DustID.CursedTorch);
                piss.velocity *= 0f;
                piss.velocity += Vector2.UnitX.RotatedByRandom(Math.PI*2)*8*Main.rand.NextFloat();
                piss.noGravity = true;
                piss.velocity += projectile.velocity * 0.1f;
                piss.scale = Main.rand.NextFloat(1.5f, 2.5f);
            }
        }
    }
