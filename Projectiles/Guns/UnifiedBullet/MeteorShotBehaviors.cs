...
{
    public class BulletBounce : BulletBehavior
    {
        private int bouncesLeft = 0;

        public BulletBounce(int bounces)
        {
            bouncesLeft = bounces;
        }
    ...
    public class MeteorTrail : BulletBehavior
    {
        private Color dustColor = new Color(0.2f, 0.0f, 0.5f, 0.2f);
        //private Color dustColor = new Color(0.6f, 0.0f, 0.2f, 0.35f);
        public override void AI(ref Projectile projectile)
        {
            if (projectile.ai[1] > 5)
            {
                Dust flame = Dust.NewDustDirect(projectile.position + new Vector2(-2,-2),0,0,DustID.Torch);
                flame.velocity *= 0.5f;
                flame.velocity += projectile.velocity * 0.5f;
